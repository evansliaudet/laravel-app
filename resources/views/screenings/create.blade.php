<x-app-layout>
    <form method="POST" action="{{ route('screening.store') }}" class="flex items-center justify-center p-5 h-screen">
        @csrf
        <div class="max-w-sm w-full flex flex-col gap-5 items-start">
            <h1 class="text-4xl font-bold">{{ __('New Screening') }}</h1>
            ...
            <p class="flex flex-col w-full">
                ...
                <select name="room_id" id="room_id" required class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                    @foreach($rooms as $room)
                        <option value="{{ $room->id }}">{{ $room->cinema->name }} - {{ $room->name }}</option>
                    @endforeach
                </select>
            </p>
            ...
            <p class="flex flex-col w-full">
                ...
                <x-input type="datetime-local" name="screening_time" id="screening_time" required />
            </p>
